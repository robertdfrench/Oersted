...
template class Coincident<CircularArc>;
...
template class Coincident<LineSegment>;

// Distance
template <>
size_t Distance<CircularArc>::set_equation_index(size_t i) { EquationIndex = i; return 1; };
...
template class Distance<CircularArc>;

template <>
size_t Distance<LineSegment>::set_equation_index(size_t i) { EquationIndex = i; return 2; };
...
template class Distance<LineSegment>;

template <>
size_t Distance<Vertex>::set_equation_index(size_t i) { EquationIndex = i; return 1; }
...
template class Distance<Vertex>;
...
void Symmetry::update(Eigen::MatrixXd &J, Eigen::VectorXd &r) {
	...
	r(EquationIndex + 1) = (((x0 - xs) * (y0 - ye) - (x0 - xe) * (y0 - ys)) + ((x1 - xs) * (y1 - ye) - (x1 - xe) * (y1 - ys))) * scale;
